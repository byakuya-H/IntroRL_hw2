...
class Make_Env(object):
    def __init__(self, env_mode=2):
        ...
        if env_mode == 1:
            ...
            # fmt: off
            self.blocks = [
                3, 4, 20, 22, 25, 26, 32, 35, 40, 41, 42, 44, 45, 47,
                61, 63, 64, 65, 71,
            ]
            # fmt: on
        ...
        for pos in self.blocks:
            ...
            self.image_init[
                :,
                (self.margin + x * self.grid_size) : (
                    self.margin + (x + 1) * self.grid_size
                ),
                (self.margin + y * self.grid_size) : (
                    self.margin + (y + 1) * self.grid_size
                ),
            ] = self.image_wall
    ...
    def not_wall_position(self, pos_man):
        if (
            pos_man[0] < 0
            or pos_man[0] == self.grid_num
            or pos_man[1] < 0
            or pos_man[1] == self.grid_num
        ):
            return 0
